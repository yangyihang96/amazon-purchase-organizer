...
class OrganizeOrdersTests(unittest.TestCase):
    ...
    def test_cli_writes_json_and_html_report(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn('grid-template-areas:"month amount" "bar bar" "count count"', html)
            self.assertIn(".month-row>b{grid-area:month", html)
            self.assertIn(".monthly-bar-track{grid-area:bar", html)
            self.assertIn(".month-count{grid-area:count;white-space:normal", html)
